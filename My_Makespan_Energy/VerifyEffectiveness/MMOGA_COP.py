...
class MMOGA_COP:
    ...
    def run(self):
        ...
        for i in range(self.subPopNum):
            self.fast_non_dominated_sort(self.P_population[i])
            for j in range(1, self.F_rank.__len__()):
                self.crowding_distance_assignment(self.F_rank[j])
            self.update_EP_FromSet(self.EP, self.P_population[i])

        self.t = 1
        while (self.t <= self.maxGen):

            threadPool = []
            for index in range(self.subPopNum):
                t = Thread(target=self.offspring, args=(index+1,))
                t.start()
                threadPool.append(t)
            ...
            self.immigrant()
            self.DTS(self.P_population)
            for subpopulation in self.P_population:
                self.update_EP_FromSet(self.EP,subpopulation)

            print('Generation ', self.t, ': ', self.IGDValue)
            self.IGD_list.append(self.IGDValue)
            self.t += 1
        ...
        # self.plotNonDominatedSolution(self.EP)
    ...
    def __init__(self, popSize, maxGen, pc, pm_SMD, pm_bit, taskNumberRange):
        ...
        self.probability = []
        # self.pc = [0,0.8,0.8,0.5,0.5]
        # self.pm_SMD = [0,0.3,0.1,0.5,0.3]
        # self.pm_bit = [0,0.03,0.01,0.05,0.01]
        self.pc = [0,0.8,0.8,0.8,0.8]
        self.pm_SMD = [0,0.3,0.3,0.3,0.3]
        self.pm_bit = [0,0.01,0.01,0.01,0.01]
    ...
    def immigrant(self):
        if not self.isImmigrant : return ;
        # select elimit individuals
        F_elimits = []
        for i in range(self.subPopNum):
            F = self.fast_non_dominated_sort(self.P_population[i])
            F_elimits.append(F[0])
        # immigrant elimit individuals
        for i in range(self.subPopNum):
            subpopulation = self.P_population[i]
            for j in range(self.subPopNum):
                if j!=i :
                    subpopulation += copy.deepcopy(F_elimits[j])
            # update subpopulation
            self.P_population[i] = self.elimitSelection(subpopulation)


    def elimitSelection(self,population):

        popSize = self.popSize
        F_rank = self.fast_non_dominated_sort(population)
        new_population = []
        i = 1
        while ((new_population.__len__() + F_rank[i].__len__()) <= popSize):
            new_population += F_rank[i]
            i += 1
        cha_N = popSize - new_population.__len__()
        if (cha_N > 0):  # 补足pSize个解
            self.crowding_distance_assignment(F_rank[i])  # 新加入的 !!
            F_rank[i] = sorted(F_rank[i], key=lambda Individual: Individual.distance,
                                    reverse=True)  # 按拥挤距离的降序排序
            new_population += F_rank[i][:int(cha_N)]
        return new_population



    def offspring(self,index):
        population = self.P_population[index-1]
        popSize = len(population)
        Q_population = self.make_new_population(population,index)  # 使用父代产生子代
        self.DVFS_Schedule_Algorithm(Q_population)
        R_population = self.combine_Pt_and_Qt(population, Q_population)
        F_rank = self.fast_non_dominated_sort(R_population)
        self.update_EP_FromSet(self.EP, F_rank[0])
        new_population = []
        i = 0
        while ((new_population.__len__() + F_rank[i].__len__()) <= popSize):
            new_population += F_rank[i]
        ...
        cha_N = popSize - new_population.__len__()
        if (cha_N > 0):  # 补足pSize个解
            self.crowding_distance_assignment(F_rank[i])
            F_rank[i] = sorted(F_rank[i], key=lambda Individual: Individual.distance,
                                    reverse=True)  # 按拥挤距离的降序排序
            new_population += F_rank[i][:int(cha_N)]
        self.P_population[index-1] = new_population
        # return new_population
    ...
    def initializePopulation(self):
        for index in range(self.subPopNum):
            ...
            for i in range(self.popSize):
                ...
                for senb in self.menb.SeNBSet:
                    for smd in senb.SMDSet:
                        ...
                        if i < self.popSize / 2:
                            ...
                            # 使用PSPI初始化
                            for task in temp_smd.workflow.taskSet:
                                # 在MEC的完成时间
                                FT_i_ws = task.d_i_j_k / temp_smd.R_i_j
                                FT_i_c = task.c_i_j_k / temp_smd.coreCC[4]
                                FT_i_wr = task.o_i_j_k / temp_smd.R_i_j
                                FT_i_atMEC = FT_i_ws + FT_i_c + FT_i_wr

                                # 在Local完成的平均时间
                                FT_i_atCore1 = task.c_i_j_k / temp_smd.coreCC[1]
                                FT_i_atCore2 = task.c_i_j_k / temp_smd.coreCC[2]
                                FT_i_atCore3 = task.c_i_j_k / temp_smd.coreCC[3]
                                FT_i_atCore = (FT_i_atCore1 + FT_i_atCore2 + FT_i_atCore3) / 3

                                if FT_i_atCore < FT_i_atMEC:
                                    # 如果task在本地完成的平均时间小于在MEC完成的时间
                                    pos = random.randint(1, self.menb.H)
                                else:
                                    pos = self.menb.H + 1
                                temp_smd.workflow.position.append(pos)
                        ...
                        # #
                        # #         # 在MEC完成的能耗
                        # #         E_i_ws = FT_i_ws * temp_smd.pws_i_j
                        # #         E_i_wr = FT_i_wr * temp_smd.pwr_i_j
                        # #         E_i_atMEC = E_i_ws + E_i_wr
                        # #
                        # #         # 在Local完成的平均能耗
                        # #         P_atCore1 = temp_smd.pcc_i_j[1]
                        # #         P_atCore2 = temp_smd.pcc_i_j[2]
                        # #         P_atCore3 = temp_smd.pcc_i_j[3]
                        # #         P_atCore = (P_atCore1 + P_atCore2 + P_atCore3) / 3
                        # #         E_i_atCore = FT_i_atCore * P_atCore
                        # #         if E_i_atCore < E_i_atMEC:
                        # #             # 如果task在本地完成的平均时间小于在MEC完成的时间
                        # #             pos = random.randint(1, self.menb.H)
                        # #         else:
                        # #             pos = self.menb.H + 1
                ...
                subpopulation.append(ind)
            self.P_population.append(subpopulation)
    ...
    # 遗传操作产生新种群
    def make_new_population(self, population,index):
        new_population = []
        ...
        self.crossoverOperator(new_population,index)
        self.mutantOperator(new_population,index)
        self.calculatePopulationFitness(new_population)
        # self.DVFS_Schedule_Algorithm(new_population)
        return new_population
    ...
    # 用轮盘赌选择法
    def rouletteWheelSelectionOperation(self,population):
        selection_probability = []
        for F in self.F_rank:
            if F == None : continue
            probability = F.__len__() / self.popSize
            selection_probability.append(probability)

        for i in range(1,selection_probability.__len__()):
            selection_probability[i] += selection_probability[i-1]

        selectionResult = []
        N = self.popSize
        while (N != 0):
            rand = random.random()
            for i in range(selection_probability.__len__()):
                if rand <= selection_probability[i]:
                    selectionResult.append(random.choice(self.F_rank[i+1]))
                    break
            N -= 1
    ...
    # 交叉
    def crossoverOperator(self, population,index):
        random.shuffle(population)
        for j in range(0, len(population), 2):  # 每隔两个个体取一次
            if j == len(population): break
            ind_1 = population[j]
            try:
    ...
    # 传递过来的条件：处在不同LeNB的不同ED具有相同channel
    # 传递来的参数为：一个LeNB中ED的坐标、另一个LeNB的坐标
    # g i,(l,k)    p5页
    ...
    def OTA(self, ind):
        ...
        W = []
        Q = []
        self.moveFirstTaskInWorkflow(W, ind)

        while len(W) != 0:

            m, workflowIndex, sequence = self.findTaskFirstSending(W)
            # update Q
            Q = self.updateQ(Q, m)
            ed = ind.chromosome[workflowIndex]
            loc = ed.workflow.position[ed.workflow.sequence.index(sequence)]
            if(len(Q) >= self.N_VM):
                loc = self.reLocation(m,Q,ed)
            if loc == 0:
                earlistTask = self.earlistTaskInQ(Q)
                m.RT_i_c = earlistTask.CT_i_c
                Q.remove(earlistTask)
                Q.append(m)
            else:
                self.continueAndFindNext(ind, W, m, workflowIndex, sequence)


    def moveFirstTaskInWorkflow(self, W, ind):
            ...
            for i in range(len(workflow.sequence)):
                ...
                if pos == 4:  # The task is executed on [10,20] server. 在MEC执行
                    ...
                    if task.id == workflow.entryTask:  # 若该task为工作流的起始任务
                        task.RT_i_l = task.ST_i_l = task.FT_i_l = 0
                        task.RT_i_ws = task.ST_i_ws = 0.0  # 准备好可以进行传输的时间 和 开始进行传输的时间
                        task.FT_i_ws = task.ST_i_ws + task.d_i_j_k / ed.R_i_j  # task完成传输的时间
                        task.RT_i_c = task.ST_i_c = task.FT_i_ws  # 任务准备好进行计算的时间 和 开始进行计算的时间
                        task.FT_i_c = task.ST_i_c + task.c_i_j_k / ed.coreCC[pos]  # task完成执行的时间
                        task.RT_i_wr = task.ST_i_wr = task.FT_i_c  # task准备好返回数据的时间 和 task开始进行返回数据的时间
                        task.FT_i_wr = task.ST_i_wr + task.o_i_j_k / ed.R_i_j  # task完成数据返回的时间
                        workflow.schedule.wsTP.append(task.FT_i_ws)
                        workflow.schedule.MECTP.append(task.FT_i_c)
                        workflow.schedule.wrTP.append(task.FT_i_wr)
                    else:  # 该task为中间task
                        ...
                        if workflow.schedule.wsTP[-1] < task.RT_i_ws:
                            task.ST_i_ws = task.RT_i_ws
                            task.FT_i_ws = task.ST_i_ws + task.d_i_j_k / ed.R_i_j
                        else:
                            task.ST_i_ws = workflow.schedule.wsTP[-1]
                            task.FT_i_ws = task.ST_i_ws + task.d_i_j_k / ed.R_i_j
                        workflow.schedule.wsTP.append(task.FT_i_ws)
                        ...
                        if workflow.schedule.MECTP[-1] < task.RT_i_c:
                            task.ST_i_c = task.RT_i_c
                            task.FT_i_c = task.ST_i_c + task.c_i_j_k / ed.coreCC[pos]
                        else:
                            task.ST_i_c = workflow.schedule.MECTP[-1]
                            task.FT_i_c = task.ST_i_c + task.c_i_j_k / ed.coreCC[pos]
                        workflow.schedule.MECTP.append(task.FT_i_c)
                        ...
                        if workflow.schedule.wrTP[-1] < task.RT_i_wr:
                            task.ST_i_wr = task.RT_i_wr
                            task.FT_i_wr = task.ST_i_wr + task.o_i_j_k / ed.R_i_j
                        else:
                            task.ST_i_wr = workflow.schedule.wrTP[-1]
                            task.FT_i_wr = task.ST_i_wr + task.o_i_j_k / ed.R_i_j
                        workflow.schedule.wrTP.append(task.FT_i_wr)
                    task.energy += ed.pws_i_j * (task.FT_i_ws - task.ST_i_ws)  # 提交能耗
                    task.energy += ed.pwr_i_j * (task.FT_i_wr - task.ST_i_wr)  # 回传能耗
                    workflow.schedule.E_total += task.energy

                    # move the first task executed on MEC
                    W.append([task, workflowIndex, i])
                    break
                else:  # The task is executed on a local core.
                    task.islocal = True
                    ...
                    if task.id == workflow.entryTask:
                        task.RT_i_l = task.ST_i_l = 0
                        task.FT_i_l = task.ST_i_l + task.c_i_j_k / ed.coreCC[pos]
                    else:  # 该task为中间task
                        task.RT_i_l = self.get_RT_i_l(task, workflow)  # 设置该task的RT为其前序任务的完成时间
                        if task.RT_i_l > workflow.schedule.coreTP[pos][-1]:
                            task.ST_i_l = task.RT_i_l
                        ...
                        task.FT_i_l = task.ST_i_l + task.c_i_j_k / ed.coreCC[pos]
                    workflow.schedule.coreTP[pos].append(task.FT_i_l)
                    task.energy = ed.pcc_i_j[pos] * (task.FT_i_l - task.ST_i_l)
                    workflow.schedule.E_total += task.energy
                workflow.schedule.S[pos].append(task.id)

    def findTaskFirstSending(self, W):
        temp = W[0]
        minCT_send = temp[0].FT_i_ws
        minCT_sendIndex = 0
        for i in range(len(W)):
        ...
        m, workflowIndex, sequence = W[minCT_sendIndex]
        W.remove(W[minCT_sendIndex])
        # return task m, the index of workflow of m, and the execution location of m
        return copy.deepcopy(m), workflowIndex, sequence

    def updateQ(self, Q, m):
        return [t for t in Q if t.FT_i_c > m.FT_i_ws]

    def reLocation(self, m, Q, ed):
        earlistTask = self.earlistTaskInQ(Q)
        CT_reci = earlistTask.FT_i_c \
                  + m.c_i_j_k / ed.coreCC[4] \
                  + m.o_i_j_k / ed.R_i_j
        CT_min = CT_reci
        loc = 3
        for k in range(1,len(self.smd.coreCC)):
            CT_comp = self.smd.workflow.schedule.coreTP[k][-1]
            CT_p = self.get_RT_i_l(m,ed.workflow)
            RT_earl = min(CT_p, CT_comp)
            CT_loc = RT_earl + + m.c_i_j_k / ed.coreCC[k]
            if CT_loc < CT_min :
                loc = k-1
                CT_min = CT_loc
        return loc
    ...
    def continueAndFindNext(self, ind, W, m, workflowIndex, sequence):
        smd = ind.chromosome[workflowIndex]
        workflow = smd.workflow
...
if __name__ == "__main__":
    ...
    mmoga_cop = MMOGA_COP(popSize, maxGen, pc, pm_SMD, pm_bit, taskNumberRange)
    mmoga_cop.run()
